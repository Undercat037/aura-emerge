...
pub(crate) fn provision_from_world_set(pretend: bool, ask: bool, verbose: bool, err_inst: bool, no_sandbox: bool, off_src_regen: bool, deep: bool) -> Result<bool> {
    ...
    if !aur_missing.is_empty() {
        ...
        if !aur_install(&aur_missing, false, ask, false, false, false, no_sandbox, off_src_regen, deep) {
            overall_ok = false;
    ...
    if !resolved_aur.is_empty() {
        ...
        if aur_install(&resolved_aur, false, ask, false, false, false, no_sandbox, off_src_regen, deep) {
